...
def api_diagnosis():
    ...
    suggestions = {
        ...
        "运筹学": ["重点复习单纯形法完整计算流程", "多练对偶问题转换和互补松弛", "排队论 M/M/1 公式套用"],
        "数据结构 C": ["重点复习链表操作和边界条件", "多练 AVL 旋转和 B 树构造", "KMP next 数组手工计算"],
...
FLASHCARDS = {
    ...
    "operations-research": [
        ...
        {"front": "二人零和博弈鞍点条件", "back": "$\\max_i\\min_j a_{ij}=\\min_j\\max_i a_{ij}=V$ 时存在纯策略鞍点。不等则需混合策略。"},
        {"front": "决策论五种准则", "back": "悲观(max-min)、乐观(max-max)、乐观系数(Hurwicz: $\\alpha\\max+(1-\\alpha)\\min$)、最小后悔值(min-max regret)、等可能(Laplace)。"},
    ...
    "data-structures-c": [
        ...
        {"front": "B 树（m 阶）性质", "back": "每结点最多 m 个子树。非根结点最少 $\\lceil m/2\\rceil$ 个子树。所有叶子在同一层。关键字数=$子树数-1$。"},
        ...
        {"front": "排序稳定性速记", "back": "稳定：冒泡、插入、归并。不稳定：选择、快速、堆、希尔。原因：是否会跨越交换相同关键字。"},
        ...
        {"front": "归并排序特点", "back": "稳定，$O(n\\log n)$ 时间，$O(n)$ 额外空间。分治法：递归划分→合并有序子序列。适合外部排序。"},
    ],
}
...
def api_insights():
    ...
    suggestions_map = {
        ...
        "运筹学": ["单纯形法完整流程", "对偶问题与互补松弛", "动态规划状态转移", "排队论 M/M/1 公式"],
        "数据结构 C": ["链表操作边界条件", "AVL 旋转与 B 树构造", "KMP next 数组计算", "排序算法稳定性判断"],
